    ...
    const submitOrder = async (forceMerge = null) => {
      ...
      if (forceMerge === "merge" && mergeModal) {
        ...
        const mergedTotal = mergedItems.reduce((s, i) => s + i.price * i.qty, 0)
          + (Number(existing.taperCost) || 0)
          + (Number(mergeModal.newDraftData.taperCost) || 0);

        const mergedNotes = [existing.notes, mergeModal.newDraftData.notes].filter(Boolean).join(" | ");
        const updated = { ...existing, items: mergedItems, total: mergedTotal, notes: mergedNotes };
                ...
                <div style={{display:"flex", flexDirection:"column", gap:8}}>
                  <button style={{...s.btn("success"), padding:14, fontSize:14, width:"100%"}}
                    ...
                    <div style={{fontSize:11, fontWeight:400, marginTop:2, opacity:0.8}}>
                      Total combinado: {fmt(mergeModal.existingOrder.total + mergeModal.newDraftData.items.reduce((s,i)=>s+i.price*i.qty,0))}
                    </div>
                  </button>
